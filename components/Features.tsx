    ...
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 mb-12">
      <div className="flex items-center justify-between gap-4 mb-6">
        ...
        <a
          ...
          className="inline-flex items-center gap-2 bg-blue-600 text-white font-bold px-5 py-3 rounded-xl hover:bg-blue-700 transition"
        >
      ...
      {blueprint.fileType === "pdf" ? (
        <div className="rounded-xl overflow-hidden border border-gray-100 h-[500px]">
          <iframe
        ...
        <p className="text-gray-600">
          Preview not available. Click <b>Download</b> to view the blueprint.
        </p>
      )}
    </div>
  );
...
export function GoogleMapView({ url }: { url: string }) {
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden mb-12">
      <div className="p-6 border-b border-gray-100 flex items-center gap-3">
        <AreaChartIcon className="h-5 w-5 text-blue-600" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="flex flex-col items-center justify-center bg-blue-50 rounded-2xl p-8">
          ...
          <p className="text-center text-blue-700/70 text-sm">
            Based on a loan amount of LKR
            {(price - downPayment).toLocaleString()} after a{" "}
            {Math.round((downPayment / price) * 100)}% down payment.
          </p>
        </div>
      </div>
    </div>
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500">
                  <div className="flex text-yellow-500">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`h-3 w-3 LKR{i < review.rating ? 'fill-yellow-500' : 'text-gray-300'}`}
                      />
                    ))}
                  </div>
                  <span>•</span>
    ...
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 sticky top-24">
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <button
                key={time}
                className="px-4 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:border-blue-600 hover:text-blue-600 transition-colors"
              >
                {time}
              </button>
            ))}
        ...
        <div className="space-y-4 pt-4 border-t border-gray-100">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
            />
          </div>
          <div className="relative">
        ...
        <p className="text-[10px] text-center text-gray-400">
          By clicking request a visit, you agree to our Terms of Use and Privacy
          Policy.
        </p>
      </div>
    </div>
